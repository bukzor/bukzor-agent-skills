# fixpoint.kb -- maintenance guide

Auxiliary theory: the background order theory every stratum's
maintenance economics reduces to. Exists so `reference` and `standing`
can cite one label instead of restating a proof sketch.

- `prior:` -- none
- `ontology:` poset, lattice, completeness, monotone operator,
  iteration, fixpoint (least/greatest), pre-/post-fixed point,
  approximation, interval, precision
- `defeated by:` a misstatement of settled mathematics -- fixed by
  restating, not by argument

## What belongs here

A theorem of order theory the tower leans on, stated so a downstream
claim can cite it by label. Standing is `bare`: these are checkable
facts, and a claim here that needs a judge is in the wrong theory.

## What does NOT belong here

Anything mentioning stores, claims, evidence, or files -- those words
live downstream. This theory must read correctly to a mathematician
who has never seen the ecosystem.
