...
- the collapse cycle settles in one pass. Dropping a moot claim's
  acts removes whole islands, so no surviving claim's bounds move
  and a second round has no further collapse to find. Coloring a
  record is fold, contest, collapse, contest -- not a loop.
- re-coloring after a new act is local. Only the act's own claim can
  change color, and after it only the claims standing on it as a
  presupposition; the rest of the corpus is untouched. That is the
  derivative REGROUND asks for, and it is what lets a corpus that
  outgrows any reader be recolored in the size of the change.
...
results above: every act would recolor everything, and the collapse
