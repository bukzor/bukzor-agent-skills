...
- [ ] Score `design.ledger.kb/good-smells.kb/` against a second notation
      — 8 of its 11 entries are `standing: open` because the set was
      extracted from one design conversation, so nothing has yet had the
      chance to fail one. Pick a published alternative (Toulmin layout,
